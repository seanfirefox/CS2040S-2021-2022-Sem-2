...
public class GameTree {
    ...
    int findValue() {
        ...
        if (this.root == null) {
            ...
            return findValue_helper(Player.ONE, this.root);

        }

    }

    // runs recursively to compare values using math.min and math.max

    int findValue_helper(Player p, TreeNode node) {
        ...
        node.value = findValue_helper(other(p), node.children[0]);
            ...
            if (p == Player.ONE) {
                ...
                node.value = Math.max(
                        ...
                        findValue_helper(
                                ...
                                other(p), node.children[i]), node.value);
